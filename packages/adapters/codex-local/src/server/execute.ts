...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (configuredCodexHome == null) {
    // A pooled account home already carries its own auth; only seed the default
    // managed home when no pool account is active for this run.
    if (!poolAccountCodexHome) {
  ...
  const effectiveCodexHome = configuredCodexHome ?? poolAccountCodexHome ?? defaultCodexHome;
